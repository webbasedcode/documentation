...
## Beschrijving 
At the moment the redis cache will not close unless the server is shut down. This will cause a memory overflow. So to avoid this, the redis cache must be shut down after a certain amount of time. In which its unused.
